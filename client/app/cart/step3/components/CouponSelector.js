      ...
      <div className="vstack gap-2 coupon-list">
        {loading ? (
          ...
          allCoupons.map((coupon) => {
            const isEligible = eligibleCoupons.some(
              (c) => c.coupon_usage_id === coupon.coupon_usage_id
            );
              ...
              >
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  {isEligible && !isExpired && !isUsed ? (
                    <button
                      className={`btn ${
                        selectedCoupon?.coupon_usage_id ===
                        coupon.coupon_usage_id
                          ? "btn-primary"
                          : "btn-outline-primary"
                      } btn-sm`}
                      onClick={() => handleApplyCoupon(coupon)}
          ...
          <div className="text-center py-3 text-muted">沒有優惠券記錄</div>
        )}
      </div>
    </div>
  );
