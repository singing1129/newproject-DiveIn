        ...
        return prevOrders.map((order) => {
          if (order.id === orderId && order.details) {
            ...
            if (reviewData) {
              // 更新商品評價狀態
              if (
                newDetails.items.products &&
                newDetails.items.products.length > 0
              ) {
                newDetails.items.products = newDetails.items.products.map(
                  (product) => {
              ...
              // 更新活動評價狀態
              if (
                newDetails.items.activities &&
                newDetails.items.activities.length > 0
              ) {
                newDetails.items.activities = newDetails.items.activities.map(
                  (activity) => {
                    const itemStatus = reviewData.items.find(
                      (item) =>
                        item.type === "activity" &&
                        item.id.toString() === activity.id.toString()
                    );
                    return {
                      ...activity,
                      isReviewed: itemStatus ? itemStatus.isReviewed : false,
              ...
              // 更新租借評價狀態
              if (
                newDetails.items.rentals &&
                newDetails.items.rentals.length > 0
              ) {
                newDetails.items.rentals = newDetails.items.rentals.map(
                  (rental) => {
                    const itemStatus = reviewData.items.find(
                      (item) =>
                        item.type === "rental" &&
                        item.id.toString() === rental.id.toString()
                    );
                    return {
                      ...rental,
                      isReviewed: itemStatus ? itemStatus.isReviewed : false,
              ...
              // 更新套裝評價狀態
              if (
                newDetails.items.bundles &&
                newDetails.items.bundles.length > 0
              ) {
                newDetails.items.bundles = newDetails.items.bundles.map(
                  (bundle) => {
                    const itemStatus = reviewData.items.find(
                      (item) =>
                        item.type === "bundle" &&
                        item.id.toString() === bundle.id.toString()
                    );
                    return {
                      ...bundle,
                      isReviewed: itemStatus ? itemStatus.isReviewed : false,
  ...
  return (
    <div className={styles.ordersContent}>
            ...
            <div key={order.id} className={styles.orderCard}>
              ...
              <div className={styles.orderHeader}>
                <h3 className={styles.orderId}>
                  ...
                  {/* {order.details &&
                    order.details.orderInfo.rewardPoints > 0 && (
                      <span
                        className="badge bg-success ms-2"
                        style={{ fontSize: "12px", verticalAlign: "middle" }}
                      >
                        <i className="fa-solid fa-coins me-1"></i>{" "}
                        {order.details.orderInfo.rewardPoints} 點積分
                      </span>
                    )} */}
                </h3>
              ...
              {expandedOrderId === order.id && (
                <div className={styles.orderExpanded}>
                  ...
                  >
                    訂單ID: {order.id} | 詳情狀態:{" "}
                    {order.details ? "已加載" : "加載中..."}
                  </div>

                  {/* 加載中指示器 */}
                  {!order.details && (
                    <div className="text-center p-3">載入訂單詳情中...</div>
                  )}
                          ...
                          <div className="col-md-4">
                            ...
                            >
                              <div>
                                ...
                                {order.details.paymentInfo.methodText}
                              </div>
                              <div>
                                <span style={{ fontWeight: "bold" }}>
                                  付款狀態:
                                </span>{" "}
                                {order.details.paymentInfo.statusText}
                              </div>
                              {order.details.paymentInfo.transactionId && (
                                <div>
                                  <span style={{ fontWeight: "bold" }}>
                                    交易編號:
                                  </span>{" "}
                            ...
                            {order.details.shippingInfo ? (
                              ...
                              >
                                <div>
                                  ...
                                  {order.details.shippingInfo.method}
                                </div>
                                <div>
                                  <span style={{ fontWeight: "bold" }}>
                                    收件人:
                                  </span>{" "}
                                  {order.details.shippingInfo.recipient}
                                </div>
                                <div>
                                  <span style={{ fontWeight: "bold" }}>
                                    聯絡電話:
                                  </span>{" "}
                                  {order.details.shippingInfo.phone}
                                </div>
                                <div>
                                  <span style={{ fontWeight: "bold" }}>
                                    配送地址:
                                  </span>{" "}
                                  {order.details.shippingInfo.address}
                                </div>
                              </div>
                            ) : (
                              <p>無配送資訊</p>
                          ...
                          <div className="col-md-4">
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  <i className="fa-solid fa-coins me-1"></i>{" "}
                                  {order.details.orderInfo.rewardPoints} 點
                                </span>
                              </div>
                              <div>
                                <small className="text-muted">
                                  此筆訂單金額 NT${" "}
                                  {order.details.orderInfo.totalAmount} 可獲得{" "}
                                  {order.details.orderInfo.rewardPoints} 點積分
                                </small>
                              </div>

                              {/* 如果有優惠券使用也可以顯示 */}
                              {order.details.orderInfo.couponDiscount > 0 && (
                                <div style={{ marginTop: "8px" }}>
                                  <span style={{ fontWeight: "bold" }}>
                                    已使用優惠券:
                                  </span>
                                  <span className="ms-1">
                                    {order.details.orderInfo.couponName ||
                                      "折扣券"}
                                  </span>
                                  <span className="text-danger ms-2">
                                    -NT${" "}
                                    {order.details.orderInfo.couponDiscount}
                                  </span>
                                </div>
                              )}
                            </div>
                              ...
                              {order.details.items.bundles.map((bundle) => (
                                ...
                                >
                                  ...
                                  <div className={styles.orderDetails}>
                                    ...
                                    <p className={styles.orderDescription}>
                                      數量: {bundle.quantity} | 原價: NT$
                                      {bundle.original_total} | 優惠價: NT$
                                    ...
                                    <button
                                      ...
                                      style={{
                                        backgroundColor: bundle.isReviewed
                                          ? "#e0e0e0"
                                          : "#FF8C00",
                                        color: "white",
                                      }}
                                    >
                                      {bundle.isReviewed ? "已評價" : "評價"}
                                    </button>
                                  </div>
                                </div>
                              ))}
                      ...
                      {order.details.items.activities &&
                        order.details.items.activities.length > 0 && (
                          <div className={styles.orderSection}>
                            ...
                            <div className={styles.orderItems}>
                              {order.details.items.activities.map(
                                (activity) => (
                                  <div
      ...
      {Object.entries(reviewWindows).map(([key, isOpen]) => {
        ...
        let item;
        if (type === "product") {
          item = order.details.items.products?.find(
            (p) => p.id.toString() === itemId
          );
        } else if (type === "activity") {
          item = order.details.items.activities?.find(
            (a) => a.id.toString() === itemId
          );
        } else if (type === "rental") {
          item = order.details.items.rentals?.find(
            (r) => r.id.toString() === itemId
          );
        } else if (type === "bundle") {
          item = order.details.items.bundles?.find(
            (b) => b.id.toString() === itemId
          );
        }
              ...
              {[...Array(5)].map((_, index) => {
                ...
                return (
                  <i
                    key={index}
                    className={`fa-${
                      ratingValue <= (reviewRatings[key] || 5)
                        ? "solid"
                        : "regular"
